...
module.exports = {
  ...
  execute: async (ctx) => {
    ...
    if (!text) return reply(resultBox('TWITTER DL', ['📝 .twitter [link Twitter/X]', '🐦 Download video Twitter']));
    try {
      reply(resultBox('TWITTER 🐦', [centerText('Mengunduh...')]));
      const res = await fetch('https://api.ryzendesu.vip/api/downloader/twitter?url=' + encodeURIComponent(text));
